...
src_folder = os.path.join(base_dir, "Toofit data", "Sherwani")
dest_dir = os.path.join(base_dir, "assets", "gallery", "wedding")

os.makedirs(dest_dir, exist_ok=True)

# Remove old sherwanis images in assets/gallery/wedding/
for f in os.listdir(dest_dir):
    if f.endswith("_sherwanis.jpg") or f.endswith("_sherwanis.png"):
        try:
...
for fname in files:
    ...
    try:
        ...
        saved_count += 1
        out_name = f"wedding_{saved_count:02d}_sherwanis.jpg"
...
print(f"\nSuccessfully added {saved_count} Sherwani images to assets/gallery/wedding/")
